...
public interface OcrDetailsRepository extends CrudRepository<OcrDetails, Integer> {
	...
	public int setVmAndVrStatus(@Param("vmStatus") String vmStatus, @Param("vrStatus") String vrStatus,
			@Param("docStatus") String docStatus, @Param("ocrid") String ocrid);
	...
	public List<OcrDetails> pendingOcrStatusDetails(@Param("vmStatus") String vmStatus,
			@Param("vrStatus") String vrStatus, @Param("docStatus") String docStatus);
	...
	@Query(value = "SELECT t FROM OcrDetails t WHERE t.gateID.gateID =:gate and t.imagePresent = 0 and t.vrStatus='pending' and t.paymentStatus='completed' and t.docStatus='completed' and t.vmStatus='completed'")
