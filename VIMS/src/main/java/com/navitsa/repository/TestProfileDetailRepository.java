...
public interface TestProfileDetailRepository extends CrudRepository<TestProfileDetail, Integer> {
	...
	@Query(value ="SELECT \r\n" + 
			...
			"    parameter_code.limit_value_desc AS limit_value_desc,\r\n" + 
			"    parameter_code.operator AS operator,\r\n" + 
			"    parameter_code.limit_value AS limit_value,\r\n" + 
			"    parameter_code.min_value AS min_value,\r\n" + 
			"    parameter_code.max_value AS max_value,\r\n" + 
			...
			"FROM\r\n" + 
			"    `test_profile_header` test_profile_header\r\n" + 
			"        INNER JOIN\r\n" + 
			"    `test_profile_detail` test_profile_detail ON test_profile_header.`profile_id` = test_profile_detail.`profile_id`\r\n" + 
			"        INNER JOIN\r\n" + 
			"    `test_value_result_detail` test_value_result_detail ON test_profile_detail.`code` = test_value_result_detail.`Code`\r\n" + 
			"        INNER JOIN\r\n" + 
			"    `parameter_code` parameter_code ON test_profile_detail.`code` = parameter_code.`code`\r\n" + 
			"        INNER JOIN\r\n" + 
			"    `test_type` test_type ON parameter_code.`type_id` = test_type.`type_id`\r\n" + 
			"        LEFT OUTER JOIN\r\n" + 
			"    `test_point` test_point ON parameter_code.`test_point_id` = test_point.`test_point_id`\r\n" + 
			"        LEFT OUTER JOIN\r\n" + 
			"    `test_parameter` test_parameter ON parameter_code.`test_parameter_id` = test_parameter.`test_parameter_id`\r\n" + 
			"        LEFT OUTER JOIN\r\n" + 
			"    `test_parameter_angle` test_parameter_angle ON parameter_code.`parameter_angle_id` = test_parameter_angle.`parameter_angle_id`\r\n" + 
			"        INNER JOIN\r\n" + 
			"    `test_profile_status` test_profile_status ON test_type.`type_id` = test_profile_status.`type_id`\r\n" + 
