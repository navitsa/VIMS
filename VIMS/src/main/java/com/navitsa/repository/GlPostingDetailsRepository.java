...
public interface GlPostingDetailsRepository extends CrudRepository<GlPostingDetails, Integer> {
	...
	@Query(value = "select * from(\r\n"
			+ "SELECT gpd.type as type, irh.Cusid as customer, gph.date as date, gph.doc_No as invoice_no, dt.Document, gph.Journal_No as gl_no, gla.GlAccNo as gl_account_no, gla.GlAccountName as gl_account, if(gpd.type='C',gpd.Amount,0) as credit,if(gpd.type='D',gpd.Amount,0) as debit \r\n"
			+ "FROM gl_posting_details gpd left join glaccount gla on gpd.GlAccNo = gla.GlAccNo ,gl_posting_head gph left join incomingreceipthead irh on gph.doc_No = irh.InRecNo left join doc_type dt on gph.Doc_id = dt.Doc_id\r\n"
			+ "where gpd.Journal_No = gph.Journal_No and gph.Doc_id in ('3')\r\n" + "union\r\n"
			+ "SELECT gpd.type as type, vr.cusid as customer, gph.date as date, gph.doc_No as invoice_no, dt.Document, gph.Journal_No as gl_no, gla.GlAccNo as gl_account_no, gla.GlAccountName as gl_account, if(gpd.type='C',gpd.Amount,0) as credit,if(gpd.type='D',gpd.Amount,0) as debit \r\n"
			+ "FROM gl_posting_details gpd left join glaccount gla on gpd.GlAccNo = gla.GlAccNo ,gl_posting_head gph left join invoicehead ih on gph.doc_No = ih.invoiceNo inner join vehicle_registration vr on ih.V_Register_ID = vr.V_Register_ID left join doc_type dt on gph.Doc_id = dt.Doc_id\r\n"
			+ "where gpd.Journal_No = gph.Journal_No and gph.Doc_id in ('2')\r\n" + "union\r\n"
			+ "SELECT gpd.type as type, vr.cusid as customer, gph.date as date, gph.doc_No as invoice_no, dt.Document, gph.Journal_No as gl_no, gla.GlAccNo as gl_account_no, gla.GlAccountName as gl_account, if(gpd.type='C',gpd.Amount,0) as credit,if(gpd.type='D',gpd.Amount,0) as debit \r\n"
			+ "FROM gl_posting_details gpd left join glaccount gla on gpd.GlAccNo = gla.GlAccNo ,gl_posting_head gph left join receipthead rh on gph.doc_No = rh.recNo inner join vehicle_registration vr on rh.V_Register_ID = vr.V_Register_ID left join doc_type dt on gph.Doc_id = dt.Doc_id\r\n"
			+ "where gpd.Journal_No = gph.Journal_No and gph.Doc_id in ('1') \r\n"
			+ ") e where e.customer = :customerId and date between :fromDate and :toDate", nativeQuery = true)
	public String[][] getCustomerGLTransactionReportDetails(@Param("fromDate") String fromDate,
			@Param("toDate") String toDate, @Param("customerId") String customerId);
}
