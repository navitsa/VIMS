...
<body onload="checkStatusofDropdowns();">
	...
	<script>
		function getInvoiceIDs(str) {
			console.log(str);
			if (str == "") {

				var slctSubcat = $('#invoiceDate'), option = "";
				slctSubcat.empty();
				return;
			} else {
				$
						.ajax({
							type : 'GET',
							url : "getAPInvoiceHeadIdsByDate",
							data : {
								"date" : str
							},
							success : function(data) {
								if (data != "") {
									var slctSubcat = $('#invoiceID'), option = "";
									slctSubcat.empty();
									...
									for (var i = 0; i < data.length; i++) {
										option = option
												+ "<option value='"+data[i].apInvoiceHeadId + "'>"
												+ data[i].apInvoiceHeadId
												+ "</option>";
									}
									slctSubcat.append(option);
								} else {
									swal(
											"No AP Invoices for selected date!",
											"",
											{
												icon : "info",
												buttons : {
